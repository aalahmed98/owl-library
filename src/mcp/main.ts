// theArchive MCP server (stdio). CRITICAL: stdout is protocol-only — log to stderr.
...
const server = new McpServer({ name: "the-archive", version: "0.1.0" });
...
async function freshIndex(): Promise<ArchiveIndex> {
  if (await index.needsRefresh()) {
    ...
    console.error(`[the-archive] index rebuilt: ${n} docs`);
  }
  return index;
}
...
async function main(): Promise<void> {
  ...
  console.error(`[the-archive] MCP server ready (content root: ${CONTENT_DIR})`);
}
